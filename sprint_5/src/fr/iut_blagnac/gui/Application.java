...
public class Application extends JFrame {
	...
	public Application(String titre) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, e.getMessage() + "\nPlease delete config/init.conf and restart the Application with writing rights.", "Error", JOptionPane.ERROR_MESSAGE);
			System.exit(ERROR);
		}
		
		
		try {
			langString = ConfigFileManager.loadLangFile(confString.get("language"));
		}
		catch (IOException e) {
			JOptionPane.showMessageDialog(this, "Was unable to load the language file \"" + confString.get("language") + "\"", "Error", JOptionPane.ERROR_MESSAGE);
			System.exit(ERROR);
		}
		...
		onglets.addTab(langString.get("roles") +" "+ langString.get("advisors"), advisorsTab);
		...
		try {
			loadedElements= OptiElementManager.open(this, new File(confString.get("etudiants")));
		...
		try {
			loadedElements= OptiElementManager.open(this, new File(confString.get("intervenants")));
		...
		/*
		try {
			loadedElements= OptiElementManager.open(this, new File(confString.get("projets")));
		...
		*/
		...
		try {
			loadedElements= OptiElementManager.open(this, new File(confString.get("sujets")));
	...
	private JMenuBar barreMenus() {
		...
		final JMenuItem saveEtudiants = new JMenuItem(langString.get("students") + "...");
		final JMenuItem saveIntervenants = new JMenuItem(langString.get("advisors") + "...");
		...
		for (final String lang : langs.list()) {
			// The value of the String below has an uppercase first letter, which is why it looks kind of strange...
			langRadio = new JRadioButtonMenuItem("" + Character.toString((char) (lang.charAt(0) - 32)) + lang.substring(1, lang.lastIndexOf(".lang")));
			buttonGroup.add(langRadio);
			langues.add(langRadio);
			
			// If the actual language is the one the application is currently in :
			if (Application.confString.get("language").equals(lang.substring(0, lang.lastIndexOf(".lang"))))
			...
			langRadio.addItemListener(new ItemListener() {			
				...
				public void itemStateChanged(ItemEvent evt) {
					if (evt.getStateChange() == ItemEvent.SELECTED) {
						try {
							ConfigFileManager.changeLanguage(lang.substring(0, lang.lastIndexOf(".lang")));
							JOptionPane.showMessageDialog(mainFrame, Application.langString.get("dialrestartnotice"), Application.langString.get("dialconfirm"), JOptionPane.INFORMATION_MESSAGE);
						}
						catch (IOException e) {
							JOptionPane.showMessageDialog(mainFrame, "An error has occured while modifying the config file.\nPlease delete config/init.conf and restart the Application with writing rights", "Error", JOptionPane.ERROR_MESSAGE);
						}
					}
				}
			});
		...
		saveEtudiants.addActionListener(new ElementsLoadNSave(saveEtudiants, currentDataManager, currentDataManager.getEtudiants(), Etudiant.class,  false));
		saveEtudiants.addActionListener(dataUpdateListener);
		loadEtudiants.addActionListener(new ElementsLoadNSave(loadEtudiants, currentDataManager, currentDataManager.getEtudiants(), Etudiant.class, true));
		loadEtudiants.addActionListener(dataUpdateListener);
		
		saveIntervenants.addActionListener(new ElementsLoadNSave(saveIntervenants, currentDataManager, currentDataManager.getIntervenants(), Intervenant.class, false));
		saveIntervenants.addActionListener(dataUpdateListener);
		loadIntervenants.addActionListener(new ElementsLoadNSave(saveIntervenants, currentDataManager, currentDataManager.getIntervenants(), Intervenant.class, true));
		loadIntervenants.addActionListener(dataUpdateListener);
		
		saveProjets.addActionListener(new ElementsLoadNSave(saveProjets, currentDataManager, currentDataManager.getProjets(), Projet.class, false));
		saveProjets.addActionListener(dataUpdateListener);
		loadProjets.addActionListener(new ElementsLoadNSave(loadProjets, currentDataManager, currentDataManager.getProjets(), Projet.class, true));
		loadProjets.addActionListener(dataUpdateListener);
		
		saveSujets.addActionListener(new ElementsLoadNSave(saveSujets, currentDataManager, currentDataManager.getSujets(), Sujet.class, false));
		saveSujets.addActionListener(dataUpdateListener);
		loadSujets.addActionListener(new ElementsLoadNSave(saveSujets, currentDataManager, currentDataManager.getSujets(), Sujet.class,  true));
		loadSujets.addActionListener(dataUpdateListener);
		
		saveAll.addActionListener(new ElementsLoadNSave(saveSujets, currentDataManager, currentDataManager.getSujets(), Sujet.class, false));
		saveAll.addActionListener(new ElementsLoadNSave(saveProjets, currentDataManager, currentDataManager.getProjets(), Projet.class, false));
		saveAll.addActionListener(new ElementsLoadNSave(saveIntervenants, currentDataManager, currentDataManager.getIntervenants(), Intervenant.class, false));
		saveAll.addActionListener(new ElementsLoadNSave(saveEtudiants, currentDataManager, currentDataManager.getEtudiants(), Etudiant.class,  false));
	...
	private void fermer() {
		...
		int reponse = JOptionPane.showOptionDialog(this,
				langString.get("dialexit"), langString.get("dialconfirm"),
				JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, new String[]{langString.get("dialyes"), langString.get("dialno")}, langString.get("dialno"));
	...
	public static void main(String[] args) throws FontFormatException, IOException {
		Application fen = new Application("Sprint 3");
		fen.setVisible(true);
	}
...
class ElementsLoadNSave implements ActionListener{
	...
	public ElementsLoadNSave(JMenuItem menuItem, DataManager dataManager, ArrayList<OptiElement> elementsList, Class<?> elementType,  boolean load) {
		this.load = load;
	...
	private void load(){
		...
		switch(chooser.showOpenDialog(this.currentMenuItem)) {
			case JFileChooser.APPROVE_OPTION:
				try {
					OptiElement[] loadedElements = OptiElementManager.open(this.currentMenuItem, chooser.getSelectedFile());
					if(loadedElements.length==0){
						JOptionPane.showMessageDialog(currentMenuItem, "No elements found in the given file", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}
					
					if(!loadedElements[0].getClass().equals(currentElementType)){
						JOptionPane.showMessageDialog(currentMenuItem, "Wrong csv file, data not matching (trouver une phrase) ", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}
			...
			case JFileChooser.ERROR_OPTION:
				JOptionPane.showMessageDialog(currentMenuItem, "Unable to load the selected file", "Error", JOptionPane.ERROR_MESSAGE);
				break;
		};
	...
	private void save(){
			JFileChooser chooser = new JFileChooser();
			if(load)
				chooser.setDialogTitle("Chargement des " + this.currentElementType.getSimpleName());
			else
				chooser.setDialogTitle("Sauvegarde des " + this.currentElementType.getSimpleName());
			...
			switch(chooser.showSaveDialog(chooser)) {
				case JFileChooser.APPROVE_OPTION:
					...
					try{
						OptiElementManager.save(this.currentMenuItem, chooser.getSelectedFile(), data);	

					}catch (FileNotFoundException e) {
							JOptionPane.showMessageDialog(this.currentMenuItem, "Unable to access the selected file", "Error", JOptionPane.ERROR_MESSAGE);
					}
					catch (IOException e) {
						JOptionPane.showMessageDialog(this.currentMenuItem, e, "Unknown Error", JOptionPane.ERROR_MESSAGE);
					}
					break;
				case JFileChooser.ERROR_OPTION:
					JOptionPane.showMessageDialog(this.currentMenuItem, "Unable to write in the selected file", "Error", JOptionPane.ERROR_MESSAGE);
					break;
			};
		}
}
